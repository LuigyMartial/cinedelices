...
module.exports = {
  async up (queryInterface, Sequelize) {
    ...
    if (!adminUser || !regularUser) {
      throw new Error('Required users are missing. Run the user seeder first.');
    }

    if (!appetizer || !mainCourse || !dessert || !beverage) {
      throw new Error('Required categories are missing. Run the category seeder first.');
    }

    if (!ratatouilleMedia || !godfatherMedia || !juliaMedia || !breakingBadMedia || !gotMedia || !pulpMedia || !amelieMedia || !chocolatMedia || !bigNightMedia || !babetteMedia || !eatDrinkMedia || !budapestMedia || !jiroMedia || !wonkaMedia) {
      throw new Error('Required media entries are missing. Run the media seeder first.');
    }
    ...
    const existingRecipes = await queryInterface.sequelize.query(
      `SELECT title FROM recipes WHERE title IN (:titles)`, {
        replacements: { titles: recipes.map((recipe) => recipe.title) },
        type: Sequelize.QueryTypes.SELECT
      }
    );

    const existingTitles = new Set(existingRecipes.map((recipe) => recipe.title));
    const recipesToInsert = recipes.filter((recipe) => !existingTitles.has(recipe.title));
  ...
  async down (queryInterface, Sequelize) {
    // Delete in reverse order to respect foreign keys
      await queryInterface.bulkDelete('recipes', {
        title: [
          'Chef Gusteau\'s Ratatouille',
          'Sicilian Cannoli from The Godfather',
          'Julia Child\'s Boeuf Bourguignon',
          'Los Pollos Hermanos Chicken',
          'Winterfell Pigeon Pie',
          'The Big Kahuna Burger',
          'Amélie\'s Crème Brûlée',
          'Chocolate Truffles',
          'Big Night Timpano',
          'Babette\'s Feast',
          'Father Chu\'s Dim Sum',
          'Mendl\'s Courtesan au Chocolat',
          'Jiro\'s Omakase Sushi',
          'Wonka Chocolate Bar'
        ]
      }, {});
  }
